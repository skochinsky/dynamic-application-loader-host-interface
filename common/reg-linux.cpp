...
public:
	static TEE_TRANSPORT_TYPE getTransportType()
	{
		...
		else if (s_transport == "AUTO")
			t_transport = TEE_TRANSPORT_TYPE_AUTO;
	...
	static string getDaemonSocketPath()
	{
		...
		// Prints too much
		// TRACE1("Daemon socket path: %s", path.c_str());
	...
	static string getAppletsDir()
	{
		...
		TRACE1("Applets dir path: %s", path.c_str());
	...
	static string getAppRepoDir()
	{
		...
		TRACE1("Applet repository dir path: %s", path.c_str());
...
JhiQueryTransportTypeFromRegistry(uint32_t *transportType)
{
	*transportType = ConfigFile::getTransportType();
	...
	if(*transportType == TEE_TRANSPORT_TYPE_AUTO)
	{
		TRACE0("Transport type AUTO configured. Checking for mei_dal presence...");
		struct stat buf;
		if(stat("/dev/dal0", &buf) == 0) // Not robust as the file path is explicitly specified but for now it's better
										 // than creating a dependency on the internals of the transport layer.
		{
			TRACE0("mei_dal found. Using transport type KERNEL");
			*transportType = TEE_TRANSPORT_TYPE_DAL_DEVICE;
		}
		else
		{
			TRACE0("mei_dal not found. Using transport type MEI");
			*transportType = TEE_TRANSPORT_TYPE_TEE_LIB;
		}
	}
...
// Currently there is only one log target on Linux: syslog/journalctl
JHI_RET_I
