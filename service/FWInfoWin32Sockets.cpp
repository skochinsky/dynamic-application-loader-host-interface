...
namespace intel_dal
{
	...
	bool FWInfoWin32Sockets::GetPlatformType(ME_PLATFORM_TYPE* platform_type)
	{
		// no amulet for VLV
		platform_type->Fields.IS_SEC = 0; //(ME)
		return true;
	}

#ifdef IPT_UB_RCR

	bool FWInfoWin32Sockets::getVproEnabled(bool* vpro)
	{
		DWORD msgCount;
		MBMessage request_msg;
		MBMessage response_msg;
		...
		// build the message.
		MBMessageBuild(&request_msg, "QRYREP", JHI_MAILBOX_NAME, DEVPLATFORM_MAILBOX_NAME, JHI_VPRO_ENABLED_REQUEST);
		...
		if (msgCount > 0)
		{
			...
			if (sscanf_s(response_msg.data,"IPT RCR: Vpro enabled =  %d",&value) != 1)
			{
				TRACE0("recieved invalid value from devplatform\n");
				return false;
			}
		}
		else
	...
	bool FWInfoWin32Sockets::getCpuBrandString(char brandString[MAX_BRAND_STRING])
	{
		DWORD msgCount;
		MBMessage request_msg;
		MBMessage response_msg;
		...
		// build the message.
		MBMessageBuild(&request_msg, "QRYREP", JHI_MAILBOX_NAME, DEVPLATFORM_MAILBOX_NAME, JHI_CPU_BRAND_STRING_REQUEST);
		...
		if (msgCount > 0)
		{
			...
			if (strcpy_s(brandString,MAX_BRAND_STRING,response_msg.data) != 0)
			{
				TRACE0("recieved invalid value from devplatform\n");
				return false;
			}
	...
	bool FWInfoWin32Sockets::getCpuType(int* cpuType)
	{
		DWORD msgCount;
		MBMessage request_msg;
		MBMessage response_msg;
		...
		// build the message.
		MBMessageBuild(&request_msg, "QRYREP", JHI_MAILBOX_NAME, DEVPLATFORM_MAILBOX_NAME, JHI_CPU_TYPE_REQUEST);
		...
		if (msgCount > 0)
		{
			...
			if (sscanf_s(response_msg.data,"IPT RCR: CPU_Type =  %d",cpuType) != 1)
			{
				TRACE0("recieved invalid value from devplatform\n");
				return false;
			}
	...
	bool  FWInfoWin32Sockets::getChipset(int* chipset)
	{
		DWORD msgCount;
		MBMessage request_msg;
		MBMessage response_msg;
		...
		// build the message.
		MBMessageBuild(&request_msg, "QRYREP", JHI_MAILBOX_NAME, DEVPLATFORM_MAILBOX_NAME, JHI_CHIPSET_REQUEST);
		...
		if (msgCount > 0)
		{
			...
			if (sscanf_s(response_msg.data,"IPT RCR: Chipset = %d",chipset) != 1)
			{
				TRACE0("recieved invalid value from devplatform\n");
				return false;
			}
